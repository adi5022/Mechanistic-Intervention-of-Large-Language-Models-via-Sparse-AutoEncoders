% ---------------------------------------------------------------------------
% DRAFT v1 (reference draft). Every experimental claim in this file is traceable
...
\begin{abstract}
A language model can give a correct completion non-trivial probability and still rank it below competing tokens. We study whether such suppressed completions can be corrected at inference time, without changing weights, by editing features of a pretrained sparse autoencoder (SAE) in the residual stream of GPT-2 small. Candidate features are ranked by their measured effect on token probabilities, and selected features are scaled down (mute) or up (boost) with a delta patch that leaves the SAE reconstruction error untouched. This paper documents the experiments completed so far. On 22 suppressed facts, iterative ablation of the target's supporting features corrected 1 fact and ablation of the top competitor's features corrected 3. On one prompt, seven competing tokens mapped to two top-ranked features, and the ten highest-ranked features of all competitors were shared. In a hybrid mute-and-boost sweep, drawing candidate features from all prompt positions instead of the last token reached rank 1 on 10 of 13 comparable prompts versus 4 of 13. A per-feature safety filter mattered: removing it lowered the rate of reaching rank 1 from 29 to 19 of 45 prompts, while relaxing it (tolerance or reduced strength) added at most one prompt across the two studies. All results are for one model, one SAE release, mostly one layer, and prompts chosen by the authors. No comparison with other steering or model-editing methods has been performed.
\end{abstract}
...
A language model can assign non-trivial probability to a correct completion and still rank it below other tokens. We call such a completion \emph{suppressed}. Repairing it with a weight edit changes the model for every later prompt~\cite{meng2022rome}. This paper studies a transient alternative: change the residual stream during one forward pass and discard the change afterwards. The direction of the change comes from a pretrained sparse autoencoder (SAE), so that one concept-like component can be scaled down or up while the rest of the activation is left as it was.

The work is an ongoing project whose repository is named FeatureScalpel. It started as a causal feature selector for a single prompt and grew into a pipeline with a safety filter, a sweep over combinations of features, and a batch evaluation harness. This paper records what has been implemented and what the experiments so far show. It is not a comparison against other methods: no experiment in the repository compares this approach with weight editing, steering vectors, prompting, or any other baseline (Section~\ref{sec:future}).

The experiments reported here are of three kinds. First, development experiments on 22 suppressed facts and on one prompt, including one hypothesis that was rejected (H10). Second, a layer sweep, a study of where candidate features are drawn from, and a study of the safety filter. Third, measurements of the cost of evaluating candidates and how batching changed it. Each result is stated with the number of prompts it rests on, and Section~\ref{sec:limits} lists what these numbers do not show.
...
\subsection{Weighted multi-competitor reduction}
\label{sec:weighted}
Two functions in \code{src/editing.py} mute the features that drive all competitors together. For each competitor $c$ ranked above the target, the weight is $w_c=p_c/\sum_{c'}p_{c'}$ and the mute strength is $w_c\,\mu_{\max}$. In the single-feature version each competitor contributes to its top-ranked driving feature $\phi(c)$, and the strength of a feature is $\min\!\big(\sum_{c:\phi(c)=i} w_c\mu_{\max},\,\mu_{\max}\big)$. The multi-feature version spreads $w_c$ over the top $K$ driving features of each competitor with equal weights, weights proportional to $\lvert\Delta\rvert$ (delta-normalized), or a softmax over $\lvert\Delta\rvert$. These functions were used for the single-prompt experiments in Section~\ref{sec:cambridge}. The interactive tabs that exposed them are no longer part of the application, and the additive aggregation described above is the version currently in the code.

\section{System Architecture}
\label{sec:arch}
Figure~\ref{fig:arch} shows the components that exist in the code. The clean pass caches the residual stream at the hook point for all positions. The candidate stages, the safety filter, and the sweep are in \code{src/editing.py}, \code{src/batched_eval.py}, and the hybrid runner. The hooks are in \code{src/hooks.py}. A batch runner executes lists of prompts under several filter settings and produces the paired comparisons in Section~\ref{sec:filter}. The application also contains a language-model explanation feature and monosemanticity diagnostics; neither is evaluated in this paper.
...
All experiments use GPT-2 small (117M parameters, 12 layers, $d_{\text{model}}=768$~\cite{radford2019gpt2}) loaded with TransformerLens defaults, and the pretrained SAE for \code{blocks.L.hook_resid_pre}, mainly $L=8$. The SAE metadata reports training on OpenWebText with context size 128 and BOS prepended. The safety-filter studies of Section~\ref{sec:filter} ran on an NVIDIA GTX 1660 Ti with PyTorch 2.6.0 and CUDA 12.4, as recorded in their study manifests. The result file of the candidate-source study records the device only as CUDA. Hardware differs between some earlier journal entries (Section~\ref{sec:limits}).

\subsection{Prompts}
\label{sec:prompts}
\emph{Fact bank.} \code{src/fact_bank.py} holds 45 (prompt, target) pairs from ROME \code{known_1000}, all with single-token targets. The rule of Section~\ref{sec:problem} labeled 15 as already correct, 8 as absent, and 22 as suppressed. The 22 suppressed facts form the evaluation set of Section~\ref{sec:stage1}, and the 15 already-correct facts serve as control prompts.
...
\emph{Candidate-source prompts.} A set of 17 prompts chosen by the authors: 14 with the target below rank 1 and 3 with the target already at rank 1 (Section~\ref{sec:candsrc}).
...
\label{sec:results}
\subsection{Iterative ablation on suppressed facts}
\label{sec:stage1}
The first evaluation ran iterative single-feature ablation at strength 0.3 for up to five rounds on the 22 suppressed facts, in two variants: ablating the top feature supporting the target (variant~A) and ablating the top feature driving the current top-1 token (variant~B). After a success, the same features were applied to the 15 control prompts, and specificity is the fraction of controls whose top-1 token was unchanged. Variant~A corrected 1 of 22 facts and variant~B corrected 3 of 22 (Table~\ref{tab:stage1}). Specificity was 100\% for all four successes.
...
All four successes started at clean rank 2, and 9 of the 22 facts started at rank 2. The top-1 token was ``the'' in 8 of the 22 facts, and none of those 8 succeeded. The top-1 tokens for the three variant-B successes were ``Italy'', ``P'' and ``May''. A journal narrative attributes the single variant-A success to the Eiffel Tower prompt, but the result tables list Castellacci and the fact set contains no Eiffel Tower prompt; we report the table. A second experiment on the same 22 facts ran single-shot joint ablation of the top competitor features against the clean top-1 token for strengths 0.3, 0.5, and 0.7 and batch sizes 1, 3, 5, and 10. It reached rank 1 on 2 to 4 of the 22 facts per setting, on 5 distinct facts in total, all with clean rank 2 (from the results file \code{outputs/stage_1/grid_sweep_results.csv}; there is no journal entry for this run).

\subsection{A single-prompt case: distributed support}
\label{sec:cambridge}
The prompt ``The location of Massachusetts Institute of Technology is in'' with target `` Cambridge'' has clean rank 8 and probability 1.15\%. Seven tokens rank above it: `` the'' (17.95\%), `` a'' (10.53\%), `` question'' (7.51\%), `` jeopardy'' (2.71\%), `` an'' (2.63\%), `` danger'' (1.76\%), and `` doubt'' (1.21\%). The top-ranked driving feature of each competitor was feature 313 for `` the'', `` a'', and `` an'', and feature 21169 for the other four. Muting these two features with weights from Section~\ref{sec:weighted} (strengths 0.284 and 0.119, maximum rule) left the target at rank 8 with probability 1.29\%.

\emph{Hypothesis H10.} The hypothesis was that the maximum rule under-muted shared features. Summing the contributions (feature 313 at 0.492, feature 21169 at 0.208) raised the target probability only from 1.29\% to 1.39\% and left the rank at 8, so H10 was rejected on this prompt. The entry notes that this shows the strongest identified feature alone was insufficient here, and that a distributed explanation had not yet been tested.

\emph{Hypothesis H11.} To test whether competitor support is spread over several features, we computed, for each competitor, the ratio of the probability drop from ablating its $k$-th ranked feature to that of its top feature. Probability differences after a softmax are not additive, so cumulative sums of drops were not used. Table~\ref{tab:decay} shows that `` question'' and `` doubt'' had flat profiles (second feature at 0.95 and 0.92 of the first) while `` the'', `` a'', and `` an'' fell to about 0.33 to 0.40. Among the top ten features of the seven competitors there were 18 distinct features, and all 18 were shared by more than one competitor (on average 3.89 competitors per feature). On this prompt the observations are consistent with a distributed representation for the semantic competitors, and with a strong single shared feature for the grammatical ones. They come from one prompt and are not a general statement about competitor representations.

\begin{table}[t]
\caption{Decay profile of each competitor's feature influence on the MIT prompt: probability drop of the $k$-th ranked feature divided by that of the top feature.}
\label{tab:decay}
\centering\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
Competitor & $k{=}2$ & $k{=}3$ & $k{=}5$ & $k{=}10$\\
\midrule
`` the'' & 0.399 & 0.328 & 0.281 & 0.061\\
`` a'' & 0.366 & 0.316 & 0.196 & 0.042\\
`` question'' & 0.947 & 0.537 & 0.270 & 0.121\\
`` jeopardy'' & 0.724 & 0.634 & 0.262 & 0.110\\
`` an'' & 0.333 & 0.259 & 0.174 & 0.035\\
`` danger'' & 0.708 & 0.571 & 0.285 & 0.058\\
`` doubt'' & 0.923 & 0.660 & 0.264 & 0.223\\
\bottomrule
\end{tabular}
\end{table}

\emph{Top-$K$ muting.} Spreading each competitor's weight over its top $K$ features (Table~\ref{tab:topk}) gave rank 8, 7, 6, and 7 for $K=1,2,3,5$ with equal weights. At $K=3$, equal and softmax weights reached rank 6 (1.527\% and 1.525\%), and delta-normalized weights rank 7 (1.470\%). At $K=5$ the intervention spread over 13 features and the rank fell back to 7. This is one prompt, one run per setting, and differences of one or two rank positions.

\begin{table}[t]
\caption{Top-$K$ multi-feature muting on the MIT prompt (maximum strength 0.7; clean rank 8, 1.15\%).}
\label{tab:topk}
\centering\footnotesize
\begin{tabular}{@{}cccc@{}}
\toprule
$K$ & Weighting & Target rank & Target prob.\ (\%)\\
\midrule
1 & all three & 8 & 1.386\\
2 & equal / delta-norm. / softmax & 7 / 7 / 7 & 1.427 / 1.413 / 1.426\\
3 & equal / delta-norm. / softmax & 6 / 7 / 6 & 1.527 / 1.470 / 1.525\\
5 & equal / delta-norm. / softmax & 7 / 7 / 7 & 1.342 / 1.382 / 1.344\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Hybrid mute and boost on the same prompt}
\label{sec:hybrid}
A sweep of the hybrid intervention on the MIT prompt (mute and boost strength 1.0) gave the results in Table~\ref{tab:hybrid}. With the safety filter applied, the best combinations reached rank 2. With the unfiltered candidates, three mutes (features 313, 8459, 21169) and three boosts reached rank 1 (7.33\% against 6.98\% and 6.78\%), and adding a fourth boost feature (24181) returned the target to rank 2. In that step the probability of `` the'' rose from 6.78\% to 7.04\% while that of `` Cambridge'' fell from 7.33\% to 6.74\%. Running the same four-boost edit alone on a clean model gave the identical result (6.7357\%, rank 2), which rules out leftover hook state. The rank can also improve while the target's probability falls: in one pair of settings the probability went from 4.28\% to 3.80\% while the rank went from 4 to 3, because a competitor dropped below the target. The observations show that adding features individually judged safe does not guarantee a monotone improvement, which is why the sweep reports its best step.
...
\label{sec:compute}
In a profiled layer-8 run of the layer benchmark, feature selection (about 350 ms) and safety filtering (about 1,200 ms) accounted for most of the 1.88 s total, which is why we measured their cost. For one prompt and layer with 30 candidates on each side, a journal audit counted 186 model forward passes in the original implementation, 126 after the safety filter stopped recomputing the clean baseline, and 122 after clean-pass caching. Stacking the candidates into the batch dimension reduced the same evaluation to 6 forward calls (1 clean, 1 each for competitor ranking, target ranking, competitor safety, and target safety, and 1 final), while the number of edited sequences evaluated was unchanged. The audit reports identical selected features (mute 8459, 21169, 14430; boost 3076, 19288, 8239), identical probabilities to $10^{-9}$ in the caching step, zero ordering differences across six prompts for the batched ranking, and 60 of 60 identical safety decisions on the MIT prompt. Removing the redundant clean passes cut the safety stage from 3,863 ms to 1,953 ms and total time from 6.38 s to 4.39 s in that audit, which used an RTX 4050 laptop GPU. In the application, the sequential and batched versions of one workflow took 12.16 s and 0.90 s, 11.55 s and 0.71 s, and 11.75 s and 0.58 s in three runs on one prompt, with the same result (rank 1, 13.42\%). A separate measurement on 3 prompts and 4 layers gave 150.07 s on a GTX 1660 Ti against 574.60 s on the CPU (3.83 times), with a peak of 1.48 GB of GPU memory. The hardware for the application timings is not stated in the entry.

\section{Discussion}
The experiments give a consistent, limited picture. In the development experiments, removing features that support the target was not a route to correction (1 of 22), and removing features that drive the blocker was a route only for a few facts (3 of 22), all of which started at rank 2. On the one prompt analyzed in detail, changing the aggregation rule for shared features did not change the rank, while spreading the mute over the top three features per competitor moved the target from rank 8 to 6. This is compatible with competitor support being spread over several features, but the evidence is one prompt and one run per setting, and the rank differences are one or two positions.

Two later results point at the supply of candidate features rather than at the selection rule. Drawing candidates from all positions increased the pool about fivefold and increased the number of prompts reaching rank 1 from 4 to 10 of 13, and every last-token failure ended by exhausting the active features. In the filter study, the strict filter removed few features for good (2.5 to 2.9\%), because it mostly sorts features into a mute side and a boost side, and relaxing it did not add prompts, while removing it cost prompts. These results are consistent with the size of the active feature set being the main limit of a multiplicative edit at one layer, and they do not exclude other limits. In particular, scaling cannot activate a feature that is off, and we have not tested any mechanism that could.

The metric matters. Rank and probability can move in opposite directions (Section~\ref{sec:hybrid}), so we report rank-based outcomes and treat probability changes as secondary. The layer sweep suggests layer 8 is a reasonable choice, but on 12 prompts it is close to its neighbors, and the sweep used a different candidate mode and strengths from the later studies.
...
\item The prompts were chosen by the authors, are few (22, 12, 13, 45, and 131 depending on the experiment), share templates, and are not independent samples. Statistical tests treat prompts as independent and are uncorrected for multiple comparisons.
...
\item The hypotheses on distributed competitor support (H10, H11) and top-$K$ muting rest on one prompt. Some other entries of the hypothesis log cite a conversation export that is not in the repository (H6 to H8), and for these no raw output is available.
\item Journal inconsistencies remain. The journals give 46 facts where the code and the category counts give 45. The stage-1 narrative names the Eiffel Tower prompt as the variant-A success while the tables name another fact, and its statistics on factual versus grammatical competitors do not match the per-fact table. The layer benchmark's own \code{success} flag counts an improvement in rank or probability and is not used here. The hardware differs between entries (GTX 1660 Ti and RTX 4050 laptop GPU), and the PyTorch version in \code{requirements.txt} (2.13.0) differs from the one used for the studies (2.6.0 with CUDA 12.4).
...
\section{Future Work}
\label{sec:future}
The following are planned or open and have \emph{not} been done. (1) Comparisons on the same prompts with weight editing~\cite{meng2022rome}, steering vectors, and prompting; the repository contains no code or results for these. (2) Testing other layers and combined layers with the same pipeline, and other models. (3) Re-selecting the blocker within a round when the top-1 token changes; in the current sweep it is fixed for the round. (4) A larger side-effect evaluation than the 20-prompt KL. (5) Mechanisms that can use features that are inactive on the prompt. (6) A learned predictor of feature effects to replace per-candidate evaluation, and gradient-based attribution as a faster approximation of the ablation ranking. (7) Repeating the all-positions safety-filter study on the full 131 prompts (only the pilot subset has been run).

\section{Conclusion}
We implemented a transient residual-stream edit through SAE features of GPT-2 small and evaluated it on suppressed factual completions. Ablating competitor features corrected 3 of 22 facts, ablating target features 1 of 22, and the single-prompt analyses were consistent with support that is spread over several features. In a hybrid sweep, candidates drawn from all prompt positions reached rank 1 on more prompts than last-token candidates (10 of 13 against 4 of 13), and the per-feature safety filter was worth keeping, because removing it lost prompts while relaxing it gained at most one. These results are for one small model, mostly one layer, and prompts chosen by us, and they do not compare the method with any alternative.
