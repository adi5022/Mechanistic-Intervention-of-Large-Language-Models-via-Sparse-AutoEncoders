...
## Corpus Check
- 68 files · ~500,138 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 380 nodes · 498 edges · 53 communities (21 shown, 32 thin omitted)
...
4. `Research Log: Investigating SAE-Based Activation Manipulation` - 13 edges
5. `get_target_token_id()` - 11 edges
6. `query_groq()` - 9 edges
7. `find_max_activating_examples()` - 9 edges
8. `make_joint_ablation_hook()` - 8 edges
9. `Agent Task: Build FeatureScalpel MVP Prototype` - 8 edges
...
- `get_cached_sae()` --calls--> `load_sae_for_layer()`  [EXTRACTED]
  experiment_app.py → src/sae_utils.py
...
### Community 0 - "monosemanticity.py"
Cohesion: 0.09
Nodes (32): slow, compute_feature_similarity(), compute_sparsity_stats(), _encode_feature_activations(), find_max_activating_examples(), find_max_activating_neuron_examples(), find_most_similar_features(), get_curated_feature_registry() (+24 more)

### Community 1 - "editing.py"
Cohesion: 0.07
Nodes (48): get_cached_model_and_sae(), get_predictions(), cache_resource, check_boost_safe(), check_combination_safe(), check_target_safe(), get_top_active_features(), get_top_competitor_features() (+40 more)
...
### Community 3 - "experiment_app.py"
Cohesion: 0.09
Nodes (28): cache_data, get_cached_base_model(), get_cached_sae(), get_neuronpedia_explanation(), make_feature_hover_link(), cache_resource, Renders a structured 6-part Educational XAI Guidance Card in the UI., Renders a structured, informative empty state card when data or metadata is… (+20 more)
...
### Community 50 - "evaluation.py"
Cohesion: 0.17
Nodes (17): DataFrame, get_target_token_id(), Safely resolves a target string (e.g. ' Paris') to its token ID in the model's…, Safely resolves a target string (e.g. ' Paris') to its token ID in the model's…, check_specificity(), classify_fact(), iterative_ablate(), Evaluation metrics and scripts for measuring intervention success. (+9 more)
...
## Knowledge Gaps
- **146 isolated node(s):** `MockConfig`, `graphify`, `Workflow: graphify`, `Project Context (read this fully before writing any code)`, `What has already been done and confirmed (do not re-derive from scratch —` (+141 more)
...
- **Why does `get_target_token_id()` connect `evaluation.py` to `editing.py`, `experiment_app.py`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `make_ablation_hook()` connect `editing.py` to `evaluation.py`, `experiment_app.py`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `get_top_competitor_features()` connect `editing.py` to `evaluation.py`, `experiment_app.py`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **What connects `MockConfig`, `graphify`, `Workflow: graphify` to the rest of the system?**
  _146 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `monosemanticity.py` be split into smaller, more focused modules?**
